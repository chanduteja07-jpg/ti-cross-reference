...
For each competitor part it returns the best TI cross with a replacement-type
classification (P2P / Drop-in / Functional) and a side-by-side spec-difference
table that highlights where the TI part is better.
"""
import io
import os
import pandas as pd
...
from crossref.data_layer import build_manual_specs, parse_name_hints
from crossref.engine import find_alternatives, load_ti_pool
from crossref.emailparse import extract_parts
from crossref.compare import analyze, replacement_type

app = Flask(__name__)

RTYPE_CLASS = {"Drop-in replacement": "drop", "P2P": "p2p",
               "Functional": "func", "Functional (closest)": "func"}
VERDICT_CLASS = {"better": "v-better", "equal": "v-equal", "worse": "v-worse", "na": "v-na"}


# --------------------------------------------------------------------------- #
#  Core processing — one path for single part, list, or email
# --------------------------------------------------------------------------- #
def build_table(a):
    cols = a["columns"]
    if not cols:
...
<style>
 :root{
   --red:#e00000; --ink:#1d1d1f; --mut:#6e6e73; --line:#e5e5ea; --bg:#fbfbfd;
   --card:#ffffff; --good-bg:#e3f6ea; --good:#1a7f43; --bad-bg:#fdecec; --bad:#b3261e;
 }
 *{box-sizing:border-box} html,body{margin:0}
 ...
 a{color:inherit}
 header{background:rgba(255,255,255,.8);backdrop-filter:saturate(180%) blur(16px);
   border-bottom:1px solid var(--line);position:sticky;top:0;z-index:10}
 .hd{max-width:1120px;margin:0 auto;padding:15px 24px;display:flex;align-items:center;gap:14px}
 .logo{background:var(--red);color:#fff;font-weight:800;font-size:15px;letter-spacing:.5px;
   padding:6px 9px;border-radius:7px}
 .hd h1{font-size:17px;margin:0;font-weight:600;letter-spacing:-.01em}
 .hd .sub{margin-left:auto;color:var(--mut);font-size:12.5px}
 ...
 form.search{background:var(--card);border:1px solid var(--line);border-radius:18px;
   padding:14px;box-shadow:0 6px 24px rgba(0,0,0,.05)}
 textarea{width:100%;border:0;outline:0;resize:vertical;font:inherit;font-size:16px;
   min-height:52px;padding:10px 12px;background:transparent}
 .rowline{display:flex;gap:10px;align-items:center;border-top:1px solid var(--line);
   padding-top:12px;margin-top:6px}
 .rowline input{flex:1;border:1px solid var(--line);border-radius:10px;padding:10px 12px;font:inherit;font-size:14px}
 button{background:var(--red);color:#fff;border:0;padding:12px 24px;border-radius:11px;
   font:inherit;font-weight:600;font-size:15px;cursor:pointer;transition:.15s}
 button:hover{filter:brightness(.94)}
 .hint{color:var(--mut);font-size:12.5px;text-align:center;margin-top:12px}
 .ex{display:inline-block;background:#f0f0f3;border-radius:20px;padding:3px 11px;margin:3px;
   font-size:12.5px;color:#333;text-decoration:none}
 ...
 /* summary table */
 .summary{width:100%;border-collapse:separate;border-spacing:0;background:var(--card);
   border:1px solid var(--line);border-radius:14px;overflow:hidden;margin:10px 0 26px}
 .summary th{background:#0b0b0f;color:#fff;text-align:left;font-weight:600;font-size:12px;
   letter-spacing:.03em;text-transform:uppercase;padding:12px 16px}
 ...
 /* diff table */
 table.diff{width:100%;border-collapse:separate;border-spacing:0;margin-top:18px;font-size:13.5px}
 table.diff th,table.diff td{padding:10px 14px;text-align:left;border-bottom:1px solid #f0f0f3}
 table.diff thead th{border-bottom:1px solid var(--line);font-size:12px;color:var(--mut);
   text-transform:uppercase;letter-spacing:.03em;font-weight:600;vertical-align:bottom}
 table.diff thead th.ti{color:var(--ink)} table.diff thead th .op{color:var(--red);font-weight:700;text-transform:none;font-size:14px;letter-spacing:0}
 ...
 td.v-worse{background:var(--bad-bg);color:var(--bad)}
 td.v-equal{}
 td.v-na{color:#9a9aa0}
 .tag{font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px}
 .tag.drop{background:var(--good-bg);color:var(--good)} .tag.p2p{background:#e6efff;color:#1257c9}
 .tag.func{background:#fdf0dd;color:#a5620a}
 .unres{background:#fff;border:1px dashed #d9c2c2;border-radius:12px;padding:14px 18px;margin:8px 0 0;color:#8a5}
 .unres b{color:#a1442e}
 .err{background:#fff6f6;border:1px solid #f2cccc;border-radius:12px;padding:16px 18px;color:#a1442e}
 .tabs{display:inline-flex;background:#ececf0;border-radius:11px;padding:3px;margin-bottom:8px}
 .tabs a{padding:7px 16px;border-radius:9px;text-decoration:none;font-size:13.5px;color:#333}
 .tabs a.on{background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.08);font-weight:600}
 .muted{color:var(--mut);font-size:13px}
</style></head><body>
<header><div class="hd"><span class="logo">TI</span><h1>Diode Cross-Reference</h1>
  <span class="sub">Formula → Algorithm · ESD / TVS · {{pool_n}} TI parts · {{comp_n}} competitor parts</span></div></header>
<div class="wrap">
 ...
 {% if results %}
   ...
   <div class="card">
     ...
     <table class="diff">
       <thead><tr>
         ...
         <th>{{it['input']}}</th>
         {% for c in it['columns'] %}
           <th class="ti"><span class="op">{{c['opn']}}</span><br>
             <span class="tag {{c['rclass']}}">{{c['rtype']}}</span></th>
         {% endfor %}
       </tr></thead>
       <tbody>
       {% for row in it['table'] %}
   ...
   <div class="unres">
     ...
     <div class="muted" style="margin-top:4px">These weren’t found in the competitor database (new/unstocked part, a zener/Schottky outside ESD-TVS scope, or not a part number). Use the manufacturer box, or enter specs manually below.</div>
 ...
 {% if manual %}
   ...
   <div class="card">
     ...
     {% if manual['table'] %}
     <table class="diff"><thead><tr><th>Spec</th><th>{{manual['input']}}</th>
       {% for c in manual['columns'] %}<th class="ti"><span class="op">{{c['opn']}}</span><br><span class="tag {{c['rclass']}}">{{c['rtype']}}</span></th>{% endfor %}
     </tr></thead><tbody>
...
def index():
    ...
    return render_template_string(PAGE, q=q, mfr=mfr, results=results, manual=manual,
                                  pool_n=len(load_ti_pool()), comp_n=_comp_count())
...
def _comp_count():
    try:
        from crossref.data_layer import load_master, load_supplemental
        n = len(load_master())
        s = load_supplemental()
        return f"{(n + (len(s) if s is not None else 0)):,}"
    except Exception:
        return "94k+"
